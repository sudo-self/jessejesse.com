...
const Project = ({ repodata }) => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch("https://api.github.com/users/sudo-self", {
      headers: {
    ...
    fetch("https://api.github.com/users/sudo-self/repos", {
      headers: {
        Authorization: { tokenkey },
      },
    })
  ...
  var searchresult = data
    ...
    .filter(
      (repo) =>
        ...
        repo.name.includes(repoSearch) ||
        (repo.language && repo.language.includes(repoSearch)) ||
        (repo.language && repo.language.toUpperCase().includes(repoSearch)) ||
        (repo.language && repo.language.toLowerCase().includes(repoSearch))
    )
    .map((repo) => (
      <Link
