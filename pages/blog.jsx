...
const Blog = () => {
  ...
  React.useEffect(() => {
    setLoad(true);
    fetch("https://dev.to/api/articles?username=sudo-self", {})
      .then((res) => res.json())
      .then((data) => {
        setArticles(data);
        setLoad(false);
      ...
      articles
        ...
        .map((data) => (
          <Link key={data.id} href={data.url} passHref>
            <div className="shadow-md shodow-black-/10 dark:shadow-zinc-200/10 hover:shadow-lg dark:hover:shadow-zinc-200/10 hover:shadow-black/10 transform transition-all w-full w-max-xl p-4 my-4 rounded-lg bg-gradient-to-r dark:bg-zinc-800 bg-slate-200 cursor-pointer">
              <p className="text-md items-center flex flex-wrap text-zinc-500">
                {data.tag_list.slice(0, 4).map((tag) => (
